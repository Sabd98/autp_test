    ...
    <>
      ...
      >
            ...
            return (
              ...
              >
                <Link href={item.href} className="flex items-center gap-3">
                  ...
                  <span className={clsx(!isOpen && 'hidden')}>{item.label}</span>
                </Link>
              </Button>
            );
          })}
        ...
        <div className="p-4 border-t border-sidebar-border overflow-hidden whitespace-nowrap text-center">
          <p className={clsx('text-xs text-sidebar-foreground/70 transition-opacity', !isOpen && 'hidden')}>
            &copy; {new Date().getFullYear()} Sabda Avicenna
          </p>
          {!isOpen && <span className="text-xs text-sidebar-foreground/70 font-bold">J</span>}
        </div>
      </aside>
